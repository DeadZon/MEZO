...
def build_workflow_inputs(config: dict, soc: str, backend: str) -> dict:
    ...
    Fly workflows do NOT accept 'devices' — the pipeline auto-detects from the ROM URL.
...
def send_config_menu(user_id: int, chat_id=None, message_id=None) -> None:
    ...
    markup.row(
        InlineKeyboardButton(f"RAM: {config['ram']}GB",                             callback_data=f"toggle_ram:{soc}"),
        InlineKeyboardButton("PixelDrain: ON" if config["upload_pixeldrain"] else "PixelDrain: OFF",
                             callback_data=f"toggle_upload:{soc}"),
    )

    if soc == "snap":
        srv_text = f"Server: {config.get('snap_servers', '1')}"
        markup.row(InlineKeyboardButton(srv_text, callback_data=f"toggle_servers:{soc}"))
