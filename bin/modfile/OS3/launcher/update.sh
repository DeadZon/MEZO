...
ENABLE_DEADZONE_OS3_LAUNCHER_REPLACE="${ENABLE_DEADZONE_OS3_LAUNCHER_REPLACE:-false}"

if [[ "${ENABLE_DEADZONE_OS3_LAUNCHER_REPLACE,,}" != "true" ]]; then
    mods "Skipping OS3 launcher replacement; preserving stock ROM launcher"
    exit 0
fi
