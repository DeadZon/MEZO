...
gf_dir="$work_dir/build/baserom/images/product/etc/device_features"

# Collect all XML files via find to avoid glob expansion issues with [ -f ]
mapfile -t xml_files < <(find "$gf_dir" -maxdepth 1 -name "*.xml" 2>/dev/null)

if [ "${#xml_files[@]}" -eq 0 ]; then
    echo "[RefreshRate] SKIPPED — no device_features XML found in ${gf_dir}"
    exit 0
fi

for gfFile in "${xml_files[@]}"; do
    echo "[RefreshRate] Patching: $gfFile"
